#import "MIWSPTProtocol.h"
#import "MIWSPTPrivate.h"
#import <objc/runtime.h>
#import "MIWSPTThread.h"

static const uint32_t MIWSPTProtocolVersion1 = 1;

NSString * const MIWSPTProtocolErrorDomain = @"MIWSPTProtocolError";
...
static void _MIWS_release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
      MIWSPTProtocol *protocol = (__bridge_transfer id)objcobj;
    protocol->queue_ = NULL;
  }
}


@interface MIWSRQueueLocalIOFrameProtocol : MIWSPTProtocol
@end
@implementation MIWSRQueueLocalIOFrameProtocol
- (void)setQueue:(dispatch_queue_t)queue {
}
@end


@implementation MIWSPTProtocol


+ (MIWSPTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
  static const char MIWScurrentQueueFrameProtocolKey;
  //dispatch_queue_t queue = dispatch_get_current_queue();
    MIWSPTProtocol *currentQueueFrameProtocol = (__bridge MIWSPTProtocol*)dispatch_queue_get_specific(queue, &MIWScurrentQueueFrameProtocolKey);
  if (!currentQueueFrameProtocol) {
    currentQueueFrameProtocol = [[MIWSRQueueLocalIOFrameProtocol alloc] initWithDispatchQueue:NULL];
    currentQueueFrameProtocol->queue_ = queue; // reference, no retain, since we would create cyclic references
    dispatch_queue_set_specific(queue, &MIWScurrentQueueFrameProtocolKey, (__bridge_retained void*)currentQueueFrameProtocol, &_MIWS_release_queue_local_protocol);
    return (__bridge MIWSPTProtocol*)dispatch_queue_get_specific(queue, &MIWScurrentQueueFrameProtocolKey); // to avoid race conds
  } else {
...
- (id)init {
  return [self initWithDispatchQueue:[[MIWSPTThread shared] queue]];
}
...
- (dispatch_data_t)createDispatchDataWithFrameOfType:(uint32_t)type frameTag:(uint32_t)frameTag payload:(dispatch_data_t)payload {
  ...
  frame->version = htonl(MIWSPTProtocolVersion1);
...
- (void)readFrameOverChannel:(dispatch_io_t)channel callback:(void(^)(NSError *error, uint32_t frameType, uint32_t frameTag, uint32_t payloadSize))callback {
  ...
  dispatch_io_read(channel, 0, sizeof(PTFrame), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    if (done) {
      ...
      if (dataSize == 0) {
        callback(nil, MIWSPTFrameTypeEndOfStream, 0, 0);
        return;
      }
...
#endif
        callback([[NSError alloc] initWithDomain:MIWSPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
        return;
      }
      ...
      if (frame->version != MIWSPTProtocolVersion1) {
        callback([[NSError alloc] initWithDomain:MIWSPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
      } else {
...
- (void)readFramesOverChannel:(dispatch_io_t)channel onFrame:(void(^)(NSError*, uint32_t, uint32_t, uint32_t, dispatch_block_t))onFrame {
  [self readFrameOverChannel:channel callback:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize) {
    onFrame(error, type, tag, payloadSize, ^{
      if (type != MIWSPTFrameTypeEndOfStream) {
...
@interface _MIWSPTDispatchData : NSObject {
  dispatch_data_t dispatchData_;
}
@end
@implementation _MIWSPTDispatchData
...
@implementation NSData (MIWSPTProtocol)
...
- (dispatch_data_t)createReferencingDispatchData {
  ...
  return dispatch_data_create((const void*)self.bytes, self.length, [[MIWSPTThread shared] queue], ^{
...
+ (NSData *)dataWithContentsOfDispatchData:(dispatch_data_t)data {
  ...
  _MIWSPTDispatchData *dispatchDataRef = [[_MIWSPTDispatchData alloc] initWithDispatchData:contiguousData];
  NSData *newData = [NSData dataWithBytesNoCopy:(void*)buffer length:bufferSize freeWhenDone:NO];
#if PT_DISPATCH_RETAIN_RELEASE
  dispatch_release(contiguousData);
#endif
  static const bool kMIWSDispatchDataRefKey;
  objc_setAssociatedObject(newData, (const void*)kMIWSDispatchDataRefKey, dispatchDataRef, OBJC_ASSOCIATION_RETAIN);
...
@implementation NSDictionary (MIWSPTProtocol)
