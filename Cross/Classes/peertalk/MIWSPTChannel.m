...
#pragma mark -
@implementation MIWSPTChannel

@synthesize protocol = protocol_;


+ (MIWSPTChannel*)channelWithDelegate:(id<MIWSPTChannelDelegate>)delegate {
  return [[MIWSPTChannel alloc] initWithProtocol:[MIWSPTProtocol sharedProtocolForQueue:[[MIWSPTThread shared] queue]] delegate:delegate];
}
...
- (id)initWithProtocol:(MIWSPTProtocol*)protocol {
  if (!(self = [super init])) return nil;
  protocol_ = protocol;
  return self;
}
...
- (id)init {
  return [self initWithProtocol:[MIWSPTProtocol sharedProtocolForQueue:[[MIWSPTThread shared] queue]]];
}
...
- (id)userInfo {
  return objc_getAssociatedObject(self, (void*)&kMIWSUserInfoKey);
}

- (void)setUserInfo:(id)userInfo {
  objc_setAssociatedObject(self, (const void*)&kMIWSUserInfoKey, userInfo, OBJC_ASSOCIATION_RETAIN);
}
...
- (id<MIWSPTChannelDelegate>)delegate {
  return delegate_;
}


- (void)setDelegate:(id<MIWSPTChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
...
- (void)connectToPort:(int)port overUSBHub:(MIWSPTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^)(NSError *error))callback {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, MIWSPTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
  ...
  // Success
  NSError *err = nil;
  MIWSPTAddress *ptAddr = [[MIWSPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  if (delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
      MIWSPTChannel *peerChannel = [[MIWSPTChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block MIWSPTChannel *localChannelRef = self;
    ...
    MIWSPTAddress *address = [[MIWSPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel error:(__autoreleasing NSError**)error {
  ...
  [protocol_ readFramesOverChannel:channel onFrame:^(NSError *error, uint32_t type, uint32_t tag, uint32_t payloadSize, dispatch_block_t resumeReadingFrames) {
    if (handleError(error, type == MIWSPTFrameTypeEndOfStream)) {
      return;
    }
    ...
    if (payloadSize == 0) {
      ...
      if (!accepted) {
        ...
        [protocol_ readPayloadOfSize:payloadSize overChannel:channel callback:^(NSError *error, dispatch_data_t contiguousData, const uint8_t *buffer, size_t bufferSize) {
          ...
          if (delegate_) {
              MIWSPTData *payload = [[MIWSPTData alloc] initWithMappedDispatchData:contiguousData data:(void*)buffer length:bufferSize];
...
- (NSString*)description {
  id userInfo = objc_getAssociatedObject(self, (void*)&kMIWSUserInfoKey);
