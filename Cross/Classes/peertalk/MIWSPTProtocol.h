...
static const uint32_t MIWSPTFrameNoTag = 0;

// Special frame type that signifies that the stream has ended.
static const uint32_t MIWSPTFrameTypeEndOfStream = 0;

// NSError domain
FOUNDATION_EXPORT NSString * const MIWSPTProtocolErrorDomain;


@interface MIWSPTProtocol : NSObject
...
@interface NSData (MIWSPTProtocol)
...
@interface NSDictionary (MIWSPTProtocol)
